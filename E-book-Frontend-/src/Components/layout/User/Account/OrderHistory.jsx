...
console.log("Order Price Check:",totalPrice);
  ...
  const handleDownload = (order) => {
    ...
    doc.setFontSize(20);
    doc.text('Bookify', 12, 20);
    doc.setFontSize(12);
    doc.text('Order Invoice', 160, 20);
    ...
    doc.text(`${order.orderId}`, valueX, y);
    y += 6;
    doc.text('Status:', labelX, y);
    doc.text(`${order.order_status}`, valueX, y);
    y += 6;
    doc.text('Order Date:', labelX, y);
    doc.text(`${order.orderDate}`, valueX, y);

    // Product Table
    y += 12;
    doc.setFont(undefined, 'bold');
    doc.text('Product Details', labelX, y);
    doc.setFont(undefined, 'normal');
    doc.line(labelX, y + 2, 198, y + 2);
    doc.autoTable({
      startY: y + 4,
      head: [['Product', 'Category', 'Quantity', 'Price']],
      body: [
        [order.productName, order.category, String(order.quantity), `₹${order.payableAmount}`],
      ],
      theme: 'striped',
    ...
    doc.line(labelX, y + 2, 198, y + 2);
    y += 8;
    doc.text('Method:', labelX, y);
    doc.text(`${order.payment_Method}`, valueX, y);
    y += 6;
    doc.text('Total Amount:', labelX, y);
    doc.text(`₹${order.payableAmount}`, valueX, y);
    y += 6;
    doc.text('Payment Status:', labelX, y);
    doc.text(`${order.payment_status}`, valueX, y);

    // Shipping Address
    y += 10;
    doc.setFont(undefined, 'bold');
    doc.text('Shipping Address', labelX, y);
    doc.setFont(undefined, 'normal');
    doc.line(labelX, y + 2, 198, y + 2);
    y += 8;
    doc.text(order.address, labelX, y, { maxWidth: 180 });

    // Footer
    doc.setFontSize(11);
    doc.setTextColor(120, 120, 120);
    doc.text('Thank you for shopping with Bookify!', 12, 285);
    doc.text('For support: support@bookify.com', 12, 292);
